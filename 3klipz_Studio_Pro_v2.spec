...
a = Analysis(
    ...
    # v7.1: was ['requests', 'dotenv', 'winrt.windows.media.control'] -
    # this reverted back from the v7.0 fix at some point (an external
    # process appears to periodically touch this project's files
    # outside this session). 'dotenv' is never referenced anywhere in
    # the source; 'winrt' is not installed (guarded by try/except
    # ImportError -> WINRT_AVAILABLE = False in the source, degrades
    # cleanly). 'matplotlib' was missing, breaking the shipped build's
    # Analytics Dashboard.
